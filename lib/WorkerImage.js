    ...
    , pad: function(options){
        ...
        this.push(function(error, data, options){
            if(error){
                ...
                var rows        = [],
                    fullWith    = data.width + left + right,
                    fullHeight  = data.height + top + bottom,

                    psize       = (this._pixelSizes[data.pixel] || 'rgb'),
                    stride      = ((fullWith * psize + 3) & ~3),

                    iterations  = data.height,

                    emptyLine   = new Buffer(stride).fill(0x0),
                    leftPad     = new Buffer(left*psize).fill(0x0),
                    rightPad    = new Buffer(stride - data.width * psize - left * psize).fill(0x0),
                    padded;
                ...
                for(var i=0; i < iterations; i++){
                    ...
                    rows.push(Buffer.concat([leftPad, data.row(i), rightPad]));
                }
                ...
                this.setWrapped(new picha.Image({width:fullWith, height: fullHeight, data: padded, pixel: data.pixel}));
